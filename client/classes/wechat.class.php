...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 */
	private function http_post($url,$param,$post_file=false,$ex=false){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
		...
		if($ex){
			foreach ($ex as $opt => $value) {
				curl_setopt($oCurl, constant($opt), $value);
			}
		}
	...
	public function payTransfers( $money,$openid,$mchid,$mch_appid,$cart_files,$apikey){
		...
		$pars['mchid'] = $mchid;
		$pars['nonce_str'] = $noncestr;
		$pars['partner_trade_no'] = time() . rand(1000, 9999);
		$pars['openid'] = $openid;
		$pars['check_name'] = 'NO_CHECK';
		$pars['amount'] = $money;
		$desc = '提现';
		$pars['desc'] = empty($desc)? '提现' : $desc;
		...
		/*print_r($pars);*/
		$xml = $this->xml_encode($pars);

		$extras['CURLOPT_SSLCERT'] = $cart_files['certfile'];
		$extras['CURLOPT_SSLKEY']  = $cart_files['keyfile'];
		$extras['CURLOPT_CAINFO']  = $cart_files['rootfile'];
		$ret = $this->http_post("https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers",
			$xml,false,$extras);
		if($ret){
			$array = (array)simplexml_load_string($ret, 'SimpleXMLElement', LIBXML_NOCDATA);
